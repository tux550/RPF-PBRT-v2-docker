...
SampleSet::SampleSet(size_t size, bool initializeData) { 
	...
	// MYEDIT: REPLACE _aligned_malloc with posix_memalign
	//cPrime = (float*) _aligned_malloc(NUM_OF_COLORS * totalSize * sizeof(float), ALIGNMENT);
	int ret = posix_memalign((void**)&cPrime, ALIGNMENT, NUM_OF_COLORS * totalSize * sizeof(float));
	if (ret != 0) {
		fprintf(stderr, "Error: Unable to allocate memory for cPrime\n");
		exit(1);
	};
	...
	if(initializeData) {
		...
		//***** DATA/WEIGHTS *****//
		// MYEDIT: REPLACE _aligned_malloc with posix_memalign
		// data = (float*) std::aligned_alloc(sampleLength * totalSize * sizeof(float), ALIGNMENT);
		//pixelColors = (float*) std::aligned_alloc(NUM_OF_COLORS * samplesPerPixel * sizeof(float), ALIGNMENT);	
		ret = posix_memalign((void**)&data, ALIGNMENT, sampleLength * totalSize * sizeof(float));
		if (ret != 0) {
			fprintf(stderr, "Error: Unable to allocate memory for data\n");
			exit(1);
		};
		ret = posix_memalign((void**)&pixelColors, ALIGNMENT, NUM_OF_COLORS * samplesPerPixel * sizeof(float));
		if (ret != 0) {
			fprintf(stderr, "Error: Unable to allocate memory for pixelColors\n");
			exit(1);
		};
		...
		// MYEDIT: REPLACE _aligned_malloc with posix_memalign
		//dataMu = (float*) std::aligned_alloc(sampleLength * sizeof(float), ALIGNMENT);
		//dataSigma = (float*) std::aligned_alloc(sampleLength * sizeof(float), ALIGNMENT);
		//normalizedData = (float*) std::aligned_alloc(sampleLength * totalSize * sizeof(float), ALIGNMENT);
		ret = posix_memalign((void**)&dataMu, ALIGNMENT, sampleLength * sizeof(float));
		if (ret != 0) {
			fprintf(stderr, "Error: Unable to allocate memory for dataMu\n");
			exit(1);
		};
		ret = posix_memalign((void**)&dataSigma, ALIGNMENT, sampleLength * sizeof(float));
		if (ret != 0) {
			fprintf(stderr, "Error: Unable to allocate memory for dataSigma\n");
			exit(1);
		};
		ret = posix_memalign((void**)&normalizedData, ALIGNMENT, sampleLength * totalSize * sizeof(float));
		if (ret != 0) {
			fprintf(stderr, "Error: Unable to allocate memory for normalizedData\n");
			exit(1);
		};
		...
		// MYEDIT: REPLACE _aligned_malloc with posix_memalign
		//PDFs = (float*) std::aligned_alloc(sampleLength * MAX_NUM_OF_BINS * sizeof(float), ALIGNMENT); // OPTIMIZATION: allocate pdfs on the fly to correct size?
		ret = posix_memalign((void**)&PDFs, ALIGNMENT, sampleLength * MAX_NUM_OF_BINS * sizeof(float));
		if (ret != 0) {
			fprintf(stderr, "Error: Unable to allocate memory for PDFs\n");
			exit(1);
		};
		...
		//***** SSE BUFFERS *****// 
		// MYEDIT: REPLACE _aligned_malloc with posix_memalign
		//normColorBuffer = (float*) std::aligned_alloc(totalSize * sizeof(float), ALIGNMENT);
		ret = posix_memalign((void**)&normColorBuffer, ALIGNMENT, totalSize * sizeof(float));
		if (ret != 0) {
			fprintf(stderr, "Error: Unable to allocate memory for normColorBuffer\n");
			exit(1);
		};
		//normFeatureBuffer = (float*) std::aligned_alloc(totalSize * sizeof(float), ALIGNMENT);
		ret = posix_memalign((void**)&normFeatureBuffer, ALIGNMENT, totalSize * sizeof(float));
		if (ret != 0) {
			fprintf(stderr, "Error: Unable to allocate memory for normFeatureBuffer\n");
			exit(1);
		};
		//colorBuffer = (float*) std::aligned_alloc(totalSize * sizeof(float), ALIGNMENT);
		ret = posix_memalign((void**)&colorBuffer, ALIGNMENT, totalSize * sizeof(float));
		if (ret != 0) {
			fprintf(stderr, "Error: Unable to allocate memory for colorBuffer\n");
			exit(1);
		};
		//featureBuffer = (float*) std::aligned_alloc(totalSize * sizeof(float), ALIGNMENT);
		ret = posix_memalign((void**)&featureBuffer, ALIGNMENT, totalSize * sizeof(float));
		if (ret != 0) {
			fprintf(stderr, "Error: Unable to allocate memory for featureBuffer\n");
			exit(1);
		};
		//tempStorageBuffer = (float*) std::aligned_alloc(totalSize * sizeof(float), ALIGNMENT);
		ret = posix_memalign((void**)&tempStorageBuffer, ALIGNMENT, totalSize * sizeof(float));
		if (ret != 0) {
			fprintf(stderr, "Error: Unable to allocate memory for tempStorageBuffer\n");
			exit(1);
		};
		//weightsBuffer = (float*) std::aligned_alloc(totalSize * sizeof(float), ALIGNMENT);
		ret = posix_memalign((void**)&weightsBuffer, ALIGNMENT, totalSize * sizeof(float));
		if (ret != 0) {
			fprintf(stderr, "Error: Unable to allocate memory for weightsBuffer\n");
			exit(1);
		};
		//tempWeightsBuffer = (float*) std::aligned_alloc(totalSize * sizeof(float), ALIGNMENT);
		ret = posix_memalign((void**)&tempWeightsBuffer, ALIGNMENT, totalSize * sizeof(float));
		if (ret != 0) {
			fprintf(stderr, "Error: Unable to allocate memory for tempWeightsBuffer\n");
			exit(1);
		};

		// MYEDIT: Wrong code previously used sizeof(int) instead of sizeof(float)
		memset(normColorBuffer, 0, totalSize * sizeof(float));
		memset(normFeatureBuffer, 0, totalSize * sizeof(float));
		memset(colorBuffer, 0, totalSize * sizeof(float));
		memset(featureBuffer, 0, totalSize * sizeof(float));
		memset(tempStorageBuffer, 0, totalSize * sizeof(float));
		memset(weightsBuffer, 0, totalSize * sizeof(float));
		memset(tempWeightsBuffer, 0, totalSize * sizeof(float));
	...
	// This is meant for the global "samples" block that holds all of the input samples
		...
		// MYEDIT: REPLACE _aligned_malloc with posix_memalign
		//cDoublePrime = (float*) std::aligned_alloc(NUM_OF_COLORS * totalSize * sizeof(float), ALIGNMENT);
		ret = posix_memalign((void**)&cDoublePrime, ALIGNMENT, NUM_OF_COLORS * totalSize * sizeof(float));
		if (ret != 0) {
			fprintf(stderr, "Error: Unable to allocate memory for cDoublePrime\n");
			exit(1);
		};
...
SampleSet::~SampleSet() {
	...
	// Clean up
	// MYEDIT: REPLACE _aligned_free with std::free
	std::free(cPrime);
	...
	if(additionalDataInitialized) {
		...
		//***** SSE BUFFERS *****//
		std::free(normColorBuffer);
		std::free(normFeatureBuffer);
		std::free(colorBuffer);
		std::free(featureBuffer);
		std::free(tempStorageBuffer);
		std::free(weightsBuffer);
		std::free(tempWeightsBuffer);
...
void SampleSet::generateRandomValues(size_t size) {
	...
	size_t index = size_t(2 * r * (RANDOM_SET_SIZE - 1) * size); 
...
int* SampleSet::generateIndexes(int b, int x, int y, size_t size, size_t& numOfSamplesUsed) {
	...
	//fprintf(stdout, "SampleSet::generateIndexes INIT\n");
